...
public class MongoWithReplicasTestBase {
    ...
    private static void initializeReplicaSet(final List<IMongodConfig> mongodConfigList) throws UnknownHostException {
        ...
        try (MongoClient mongo = MongoClients.create(mo)) {
            ...
            Document cr = Multi.createFrom().publisher(mongoAdminDB.runCommand(new Document("isMaster", 1)))
            ...
            cr = Multi.createFrom().publisher(mongoAdminDB.runCommand(new Document("replSetInitiate", rsConfiguration)))
                    .collect().first().await().indefinitely();
            ...
            await()
                    ...
                    .until(() -> {
                        Document result = Multi.createFrom()
                                .publisher(mongoAdminDB.runCommand(new Document("replSetGetStatus", 1)))
                                .collect().first().await().indefinitely();
